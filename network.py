...
INPUT_DIM = 4 # входные данные
OUT_DIM = 3 # выходные данные (колво классов)
H_DIM = 5 # колво слоев

#x = np.random.randn(INPUT_DIM) # входной вектор (случайный вектор)

#w1 = np.random.randn(INPUT_DIM, H_DIM) # INPUT_DIM - колво строк, H_DIM - колво столбцов
...
def relu(t):  # максимум из предшествующего значения и 0
    return np.maximum(t, 0)

def softmax(t): # функция, которая превращает вектор в набор вероятностей
    out = np.exp(t)
    return out / np.sum(out)

def predict(x):
    t1 = x @ w1 + b1  # @ способ умножения матриц, матрицу на вектор (линейная часть)
    h1 = relu(t1)
    t2 = h1 @ w2 + b2
    z = softmax(t2)
    return z
